<template>
  ...
  <b-form
    ...
    @submit.prevent="saveUserPermission"
  >
    <b-row>
      <b-col md="12">
        <b-form-group
        ...
        >
          <b-input-group class="input-group-merge">
            ...
            <v-select
              ...
              placeholder="Select"
              label="name"
              :reduce="userType => userType.adminuserid"
              :options="user"
              @input="changeUser"
            />
...
export default {
  ...
  methods: {
    ...
    saveUserPermission() {
      store.dispatch('CompanyPermission/saveUserPermission', { companyuserid: this.userTypeSelected, transactionIds: this.permissionSelected })
        .then(response => {
          if (response.data.code === '200') {
            this.permissionSelected = response.data.data
            this.$toast({
